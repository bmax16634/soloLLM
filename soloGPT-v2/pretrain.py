...
with open("./soloGPT-v2/config.json") as f:
    config = json.load(f)

# Set up output directory and filenames
output_dir = config.get("save_path", "./outputs")
...
try:
    while total_tokens_processed < target_tokens and shard_idx < len(shard_paths):
        ...
        for batch in dataloader:
            ...
            if step_tokens >= tokens_per_checkpoint:
                ckpt_name = os.path.join(output_dir, f"{model_basename}_{total_tokens_processed//1_000_000}M.pth")
    ...
    with open(os.path.join(output_dir, "loss_curve.json"), "w") as f:
