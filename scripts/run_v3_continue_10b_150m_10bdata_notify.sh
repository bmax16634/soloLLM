...
RUN_NAME="${RUN_NAME:-v3_fresh_10b_plus_150m_1024_10bdata}"
...
RESUME_PATH="${RESUME_PATH:-${ROOT}/outputs/sologpt_v3/v3_fresh_5b_plus_150m_1024_10bdata/checkpoints/latest.pt}"
SHARD_DIR="${SHARD_DIR:-/home/bmx/_projects/soloLLM/data/v3_10b_1024/chunks}"
TRAIN_SHARDS="${TRAIN_SHARDS:-596:1168}"
VAL_SHARDS="${VAL_SHARDS:-1169:1180}"
TARGET_TOKENS="${TARGET_TOKENS:-10000000000}"
CONTINUATION_LR="${CONTINUATION_LR:-0.0001}"
...
echo "=== SoloLLM v3 continue fresh 5B to 10B on 10B data ==="
...
echo "continuation_learning_rate=$CONTINUATION_LR"
echo "pid=$$"
...
python -m sologpt_v2.pretrain \
  ...
  --learning-rate "$CONTINUATION_LR" \
  --max-eval-tokens 4194304 \
  --eval-every-tokens 250000000 \
  --tokens-per-checkpoint 250000000 \
...
details = []
title = "SoloLLM v3 10B continuation complete" if status == 0 else "SoloLLM v3 10B continuation failed"
...
if summary_path.exists():
    ...
    details.append(f"{tokens / 1_000_000_000:.2f}B total tok")
    details.append(f"{processed / 1_000_000_000:.2f}B new tok")
    if elapsed:
