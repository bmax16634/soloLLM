...
SOURCE_CKPT="${SOURCE_CKPT:-outputs/sologpt_v2/pilot_300m_modern_small_resume100m/checkpoints/latest.pt}"
...
MAX_TOKENS="${MAX_TOKENS:-3000000000}"
EVAL_EVERY_TOKENS="${EVAL_EVERY_TOKENS:-100000000}"
TOKENS_PER_CHECKPOINT="${TOKENS_PER_CHECKPOINT:-100000000}"
LOG_PATH="${LOG_PATH:-$RUN_DIR/final_3b.log}"

mkdir -p "$RUN_DIR"
exec >> "$LOG_PATH" 2>&1

echo "=== SoloLLM v2 modern-small final 3B run ==="
...
parts = ["v2-modern-small final 3B run finished"]
...
if [[ ! -f "$SOURCE_CKPT" ]]; then
  ...
  echo "$msg"
  post_reminder "SoloLLM final 3B run did not start" "$msg" "Run directory: $ROOT/$RUN_DIR"
  exit 2
fi
...
if [[ "$status" -eq 0 ]]; then
  body="$(summary_body)"
  post_reminder "SoloLLM final 3B run complete" "$body" "Run directory: $ROOT/$RUN_DIR"
else
  body="v2-modern-small final 3B run stopped with exit status $status. Check $ROOT/$LOG_PATH."
  post_reminder "SoloLLM final 3B run stopped" "$body" "Run directory: $ROOT/$RUN_DIR"
fi
